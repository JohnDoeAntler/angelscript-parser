...
export class asCTokenizer {
	...
	constructor() {
        ...
        Object.keys(this.keywordTable).forEach((e) => {
            this.keywordTable.get(e)?.sort((a, b) => a.wordLength - b.wordLength);
        });
    }
    ...
    ParseToken(source: string): Token {
        ...
        if(tmp = this.IsWhiteSpace(source)) return new Token(tmp.tokenType, tmp.length, asETokenClass.asTC_WHITESPACE);
        if(tmp = this.IsComment(source)) return new Token(tmp.tokenType, tmp.length, asETokenClass.asTC_COMMENT);
        if(tmp = this.IsConstant(source)) return new Token(tmp.tokenType, tmp.length, asETokenClass.asTC_VALUE);
        if(tmp = this.IsIdentifier(source)) return new Token(tmp.tokenType, tmp.length, asETokenClass.asTC_IDENTIFIER);
        if(tmp = this.IsKeyWord(source)) return new Token(tmp.tokenType, tmp.length, asETokenClass.asTC_KEYWORD);
    ...
    IsConstant(source: string): Omit<Token, 'tokenClass'> | null {
        ...
        if((source[0] >= '0' && source[0] <= '9') || (source[0] == '.' && source.length > 1 && source[1] >= '0' && source[1] <= '9')) {
            ...
            if(n < source.length && (source[n] == '.' || source[n] == 'e' || source[n] == 'E')) {
                if( source[n] == '.' ) {
                    n++;
                    for(; n < source.length; n++) {
                        if( source[n] < '0' || source[n] > '9' )
                ...
                if(n < source.length && (source[n] == 'e' || source[n] == 'E')) {
                    n++;
                    if(n < source.length && (source[n] == '-' || source[n] == '+'))
                        n++;
                ...
                if( n < source.length && (source[n] == 'f' || source[n] == 'F') ) {
                    return {
        ...
        if(source[0] == '"' || source[0] == "'") {
            ...
            if(source.length >= 6 && source[0] == '"' && source[1] == '"' && source[2] == '"') {
